...
.PHONY: up down build logs open clean help test test-frontend test-frontend-watch test-frontend-coverage test-backend test-backend-watch run gen-meetings run-meetings post-eval
...
## run-meetings: Run meetings from a JSON file generated by gen-meetings
MEETING ?=
MODEL ?=
MAX_TURNS ?=
TURN_RULE ?=
...
run-meetings:
ifndef MEETING
	@echo "Usage: make run-meetings MEETING=<file.json> MODEL=<model> OUTPUT=<file.json> [options]"
	@echo ""
	@echo "Required:"
	@echo "  MEETING           JSON file generated by gen-meetings"
	@echo "  MODEL             LLM(s) that instantiate participants, assigned in order and cycled"
	@echo "                    (e.g. MODEL=openai/gpt-4o-mini or MODEL=\"vllm/0/Qwen/Qwen3-8B openai/gpt-4o-mini\")"
	@echo "  OUTPUT            Save analytics of all meetings to JSON file"
	@echo ""
	@echo "Optional:"
	@echo "  MAX_TURNS         Maximum number of turns per meeting (default: 100)"
	@echo "  TURN_RULE         Turn rule for the conversation phase (default: round_robin)"
	@echo "  VOTING_RULE       Voting rule for the voting phase (default: majority)"
	@echo "  VOTE_TURN_RULE    Turn rule for the voting phase (default: round_robin)"
	@echo "  TIME_LIMIT        Time limit per meeting in seconds (default: unlimited)"
	@echo "  VOLUNTEER_MODE    true | false: allow participants to skip their own turn (default: false)"
	@echo "  BALANCED_TURNS    true | false: balance speaking turns (default: true)"
	@echo "  ENABLE_POST_EVAL  true | false: participants self-evaluate the final itinerary at the end (default: true)"
	@echo "  SINGLE_DECIDER    Index (0-based) or SINGLE_DECIDER=facilitator: decider in each meeting (with VOTING_RULE=single_decider)"
	@echo "  TRAVEL_DATE       Constraint: travel date (e.g. 2026-08-01)"
	@echo "  TIME_WINDOW_START Constraint: tour start time (e.g. 09:00)"
	@echo "  TIME_WINDOW_END   Constraint: tour end time (e.g. 18:00)"
	@echo "  BUDGET            Constraint: budget per participant (e.g. '\$$600')"
	@echo "  TEMPERATURE       Sampling temperature(s) for participant LLMs, assigned in order and cycled (default: 0.7)"
	@echo "  SEED              Random seed(s) for participant LLMs, assigned in order and cycled (default: 42)"
	@echo "  INDEX             Run only the meeting at this index (0-based; default: run all)"
	@echo ""
	@echo "Examples:"
	@echo "  make run-meetings MEETING=kyoto_tours.json MODEL=openai/gpt-4o-mini OUTPUT=kyoto_results.json"
	@echo "  make run-meetings MEETING=barcelona_tours.json MODEL=openai/gpt-4o-mini OUTPUT=barcelona_results.json MAX_TURNS=50 INDEX=0"
	@exit 1
else ifndef MODEL
	@echo "Error: MODEL is required. Example: make run-meetings MEETING=kyoto_tours.json MODEL=openai/gpt-4o-mini OUTPUT=results.json"
	@exit 1
else ifndef OUTPUT
	@echo "Error: OUTPUT is required. Example: make run-meetings MEETING=kyoto_tours.json MODEL=openai/gpt-4o-mini OUTPUT=results.json"
	@exit 1
else
	$(DOCKER_COMPOSE) exec -e PYTHONPATH=/app -e VLLM_BASE_0=http://vllm-0:8000/v1 -e VLLM_BASE_1=http://vllm-1:8000/v1 backend python3 -m tour_meeting.run_meetings \
		"$(MEETING)" --model $(MODEL) --output "$(OUTPUT)" \
		$(if $(MAX_TURNS),--max-turns $(MAX_TURNS)) \
		$(if $(TURN_RULE),--turn-rule $(TURN_RULE)) \
		$(if $(VOTING_RULE),--voting-rule $(VOTING_RULE)) \
		$(if $(VOTE_TURN_RULE),--vote-turn-rule $(VOTE_TURN_RULE)) \
		$(if $(TIME_LIMIT),--time-limit $(TIME_LIMIT)) \
		$(if $(VOLUNTEER_MODE),--volunteer-mode $(VOLUNTEER_MODE)) \
		$(if $(BALANCED_TURNS),--balanced-turns $(BALANCED_TURNS)) \
		$(if $(ENABLE_POST_EVAL),--enable-post-eval $(ENABLE_POST_EVAL)) \
		$(if $(SINGLE_DECIDER),--single-decider $(SINGLE_DECIDER)) \
		$(if $(TRAVEL_DATE),--travel-date "$(TRAVEL_DATE)") \
		$(if $(TIME_WINDOW_START),--time-window-start "$(TIME_WINDOW_START)") \
		$(if $(TIME_WINDOW_END),--time-window-end "$(TIME_WINDOW_END)") \
...
## post-eval: Participants self-evaluate the final itinerary of finished meetings
RESULTS ?=
post-eval:
ifndef MEETING
	@echo "Usage: make post-eval MEETING=<file.json> RESULTS=<file.json> OUTPUT=<file.json> [options]"
	@echo ""
	@echo "Each participant is re-instantiated with the same persona and LLM used"
	@echo "during the meeting, and scores the final itinerary (1-10, with a reason)."
	@echo ""
	@echo "Required:"
	@echo "  MEETING           JSON file generated by gen-meetings (meeting definitions)"
	@echo "  RESULTS           JSON file produced by run-meetings (analytics of finished meetings)"
	@echo "  OUTPUT            Save evaluations to JSON file"
	@echo ""
	@echo "Optional:"
	@echo "  INDEX             Evaluate only the meeting at this index (0-based; default: evaluate all)"
	@echo ""
	@echo "Examples:"
	@echo "  make post-eval MEETING=kyoto_tours.json RESULTS=kyoto_results.json OUTPUT=kyoto_eval.json"
	@exit 1
else ifndef RESULTS
	@echo "Error: RESULTS is required. Example: make post-eval MEETING=kyoto_tours.json RESULTS=kyoto_results.json OUTPUT=kyoto_eval.json"
	@exit 1
else ifndef OUTPUT
	@echo "Error: OUTPUT is required. Example: make post-eval MEETING=kyoto_tours.json RESULTS=kyoto_results.json OUTPUT=kyoto_eval.json"
	@exit 1
else
	$(DOCKER_COMPOSE) exec -e PYTHONPATH=/app -e VLLM_BASE_0=http://vllm-0:8000/v1 -e VLLM_BASE_1=http://vllm-1:8000/v1 backend python3 -m tour_meeting.post_eval \
		"$(MEETING)" "$(RESULTS)" --output "$(OUTPUT)" \
		$(if $(INDEX),--index $(INDEX))
endif
...
help:
	...
	@echo "  run-meetings         Run meetings from a JSON file generated by gen-meetings"
	@echo "  post-eval            Participants self-evaluate the final itinerary of finished meetings"
	...
	@echo "  make run SCRIPT=examples/barcelona.py ARGS='--max-turns 10'"
	@echo "  make run SCRIPT=examples/barcelona.py ARGS='--model openai/gpt-5-mini-2025-08-07'"
	@echo "  make gen-meetings INSTRUCTION='3 people touring Kyoto' GEN_MODEL=openai/gpt-4o-mini OUTPUT=kyoto_tours.json"
	@echo "  make run-meetings MEETING=kyoto_tours.json MODEL=openai/gpt-4o-mini OUTPUT=results.json   # Run generated meetings"
